...
#默认一次生成的优选ip数以passwall_config_nodes.txt中的所有passwall节点为准，运行此脚本需要先定义passwall_config_nodes.txt
#
totalips=$(cat $passwall_config_nodes | wc -l)
nodes_line=1

if [ -e $passwall_results ]; then rm $passwall_results; fi
if [ -e $serverresults ]; then rm -rf $serverresults; fi

#用shell写的Json解析函数, 非必须，预留，需要解析IP库的json格式需要，不能编译jq的路由器适用。
...
do
    ...
    do
        ...
        clear
        # curl -H POST ip-api.com/json/$anycast -o temp.txt
        ...
        # publicip=$(curl cip.cc | grep "IP"  |  awk -F':' '{print $2}')
        # colo=$(getJsonValues "$(cat temp.txt)" "org" "defaultValue")
        # city=$(getJsonValues "$(cat temp.txt)" "city" "defaultValue")
        # country=$(getJsonValues "$(cat temp.txt)" "country" "defaultValue")
        rm -rf temp.txt
        ...
        uci set passwall.$(cat $passwall_config_nodes | sed -n $nodes_line\p).address=$anycast
        uci commit passwall

        echo -e $(cat $passwall_config_nodes | sed -n $nodes_line\p) $anycast $realbandwidth Mbps >>$passwall_results
        echo -e $anycast,$colo,$realbandwidth Mbps >>$serverresults
...
if [ $passwall_enable ]; then
	/etc/init.d/haproxy restart
	/etc/init.d/passwall restart
fi

if [ $clash_enable ]; then
    cp -rf clash.yaml /etc/openclash/
